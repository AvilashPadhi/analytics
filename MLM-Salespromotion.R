...
fitted(fit2) #resultant prediction from given data set
omni #given data set
residuals (fit2) #difference between predicted and actual
...
#Assumptions
par(mfrow=c(2,2)) #plot out 4 plots at the same time
plot(fit2)
par(mfrow=c(1,1)) #only one plot after the other
plot(fit2)
plot(fit2)
